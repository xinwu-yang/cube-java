...
import org.cube.cloud.starter.lock.annotations.JLock;
...
import org.cube.cloud.starter.lock.enums.LockModel;
...
public class DistributedLockHandler extends BaseAspect {
    ...
    private RLock getLock(ProceedingJoinPoint joinPoint, JLock jLock) {
        ...
        if (lockModel.equals(LockModel.MULTIPLE) || lockModel.equals(LockModel.REDLOCK)) {
            if (keys.length == 1) {
                throw new RuntimeException("参数只有一个，锁模式为->" + lockModel.name() + "，无法锁定！");
            }
        }
        RLock rLock = null;
