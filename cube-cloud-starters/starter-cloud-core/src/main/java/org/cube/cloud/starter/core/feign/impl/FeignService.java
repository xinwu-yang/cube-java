package org.cube.cloud.starter.core.feign.impl;
...
import org.cube.cloud.starter.core.feign.IFeignService;
    ...
    public <T> T newInstance(Class<T> clientClass, String serviceName) {
        builder.requestInterceptor(requestTemplate -> {
            ...
            if (null != attributes) {
                ...
                // 将token信息放入header中
                String token = request.getHeader(CommonConst.X_ACCESS_TOKEN);
                if (token == null) {
                    token = request.getParameter("token");
                }
                log.info("Feign request token: {}", token);
                requestTemplate.header(CommonConst.X_ACCESS_TOKEN, token);
            }
        });
