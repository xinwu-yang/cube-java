...
public class YouBianCodeUtil {
    ...
    public static synchronized String getNextYouBianCode(String code) {
        ...
        if (code == null || code.equals("")) {
            ...
            if (after_code_num == getMaxNumByLength()) {
            ...
            if ('Z' == after_code_zimu && getMaxNumByLength() == after_code_num) {
...
//			return parentCode + getNextYouBianCode(localCode);
    ...
    private static int getMaxNumByLength() {
        if (YouBianCodeUtil.numLength == 0) {
            return 0;
        }
        StringBuilder max_num = new StringBuilder();
        for (int i = 0; i < YouBianCodeUtil.numLength; i++) {
            max_num.append("9");
        }
